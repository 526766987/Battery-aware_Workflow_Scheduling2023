...
class DC_MBFLS(object):
    ...
    def stage2(self):
        ...
        for i in torch.flip(self.ranku[:, 1], dims = [0]):
            ...
            if ((rce != self.ddl) & (rce > self.stage2_taskRecord[i, 0])):  # there is a scheduling space
                minISqure = self.VolatilityCal(i, 0, rce)
                # find the best time
                delta_t = 0
                for t in range(1, math.floor(rce + 1 - self.stage2_taskRecord[i, 0].item())):
                    if self.SlidingConflictCheck(i, t):
                        sI2 = self.VolatilityCal(i, t, rce)
                        if (sI2 <= minISqure):
                            minISqure = sI2
                            delta_t = t
                self.stage2_taskRecord[i, 0] = self.stage2_taskRecord[i, 0] + delta_t
                self.stage2_taskRecord[i, 1] = self.stage2_taskRecord[i, 1] + delta_t
